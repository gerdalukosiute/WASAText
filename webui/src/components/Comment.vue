...
const handleEmojiClick = async (emoji) => {
  try {
    if (userReaction.value === emoji) {
      ...
      if (commentToDelete) {
        await axios.delete(`http://localhost:8080/messages/${props.messageId}/comments/${commentToDelete.id}`, {
          headers: {
      ...
      // Add new reaction
      const response = await axios.post(`http://localhost:8080/messages/${props.messageId}/comments`, {
        content: emoji
      }, {
        headers: {
          'Content-Type': 'application/json',
